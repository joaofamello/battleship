...
class BaseAI
  ...
  #
  # O metodo percorre cada navio da frota (@fleet) e tenta, repetidamente, encontrar
  ...
  #
  # @abstract Este metodo é apenas um contrato As classes filhas (EasyBot, MediumBot e HardBot)
  # irão implementar a própria estratégia.
  #
  # @param target [Board] o tabuleiro do oponente que receberá o tiro
  # @raise [RuntimeError] Caso o metodo seja chamado diretamente na BaseAI
  # ou se a classe filha não sobrescrever este metodo.
  def shoot(target)
